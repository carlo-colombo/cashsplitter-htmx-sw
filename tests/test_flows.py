...
def wait_for_app_ready(page: Page):
    ...
    with page.expect_response("**/api/fragment/group-list") as response:
        pass # The action that triggers the request has already happened.

def test_create_group(page: Page):
    wait_for_app_ready(page)
    page.get_by_role("button", name="+").click()
    page.get_by_placeholder("e.g., Holiday Trip").fill("Test Group")
    page.get_by_placeholder("Comma-separated names").fill("Alice, Bob")
...
def test_see_details(page: Page):
    wait_for_app_ready(page)
    page.get_by_role("button", name="+").click()
    page.get_by_placeholder("e.g., Holiday Trip").fill("Details Test")
    page.get_by_placeholder("Comma-separated names").fill("Charlie, David")
    page.get_by_role("button", name="Save changes").click()
...
def test_delete_group(page: Page):
    wait_for_app_ready(page)
    page.get_by_role("button", name="+").click()
    page.get_by_placeholder("e.g., Holiday Trip").fill("To Delete")
    page.get_by_placeholder("Comma-separated names").fill("Eve")
    page.get_by_role("button", name="Save changes").click()
