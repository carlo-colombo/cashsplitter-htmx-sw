...
def test_create_group(page: Page):
    ...
    expect(page.locator("#new-group-modal").last).to_be_visible()
    page.get_by_placeholder("e.g., Holiday Trip").last.fill("Test Group")
    page.get_by_placeholder("Comma-separated names").last.fill("Alice, Bob")
    page.get_by_role("button", name="Save changes").last.click()
...
def test_see_details(page: Page):
    ...
    page.get_by_role("button", name="+").last.click()
    expect(page.locator("#new-group-modal").last).to_be_visible()
    page.get_by_placeholder("e.g., Holiday Trip").last.fill("Details Test")
    page.get_by_placeholder("Comma-separated names").last.fill("Charlie, David")
    page.get_by_role("button", name="Save changes").last.click()
...
def test_delete_group(page: Page):
    ...
    page.get_by_role("button", name="+").last.click()
    expect(page.locator("#new-group-modal").last).to_be_visible()
    page.get_by_placeholder("e.g., Holiday Trip").last.fill("To Delete")
    page.get_by_placeholder("Comma-separated names").last.fill("Eve")
    page.get_by_role("button", name="Save changes").last.click()
